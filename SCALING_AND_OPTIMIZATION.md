...
- [ ] Резервная копия БД и конфигурации
- [ ] Проверка текущего способа запуска (ts-node / node, переменные окружения)
- [ ] Зафиксировать текущие метрики PM2 (heap, restarts, uptime) для сравнения
- [ ] Включить/проверить логирование ошибок (чтобы не потерять диагностику при сокращении логов)
