...
## 🔔 Критические алерты (Обновлено: 11.02.2026)

**Автоматические алерты ОТКЛЮЧЕНЫ** - алерты отправляются только по запросу.

### Использование:
1. **Ежедневный отчет**: `npx ts-node scripts/daily-quality-report.ts`
2. **Ручная отправка**: `npx ts-node scripts/send-critical-alert-manual.ts`
3. **Просмотр проблем**: `npx ts-node scripts/view-data-issues.ts`

📖 Подробнее: `CRITICAL_ALERTS_MANUAL.md`
...
- [x] ~~Алерты админу при критических проблемах~~ (реализовано, работает в ручном режиме)
