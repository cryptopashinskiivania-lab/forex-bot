...
- **События со временем:** напоминание за **15 минут** до времени события; после выхода новости — публикация **результата** (через 5 мин после времени события). Ежедневно в **08:00** (по времени пользователя) автоматически отправляется то же сообщение, что и по команде **/daily** (сводка за сегодня и кнопки AI Forecast / AI Results).
